            ...
            //  console.log(fiveOnFive[12])
              ...
              console.log(tempArray9); // array of mutual penalties
               ...
               for (i = 0; i < tempArray9.length; i++) {tempArray10 = []; tempArray1 = []; tempArray12 = []; tempArray11 = []; tempArray13 =[]; tempArray14 = [];
                ...
                tempArray10.push(counterArray[0][2 * j]); tempArray11.push(counterArray[0][2 * j])}
                else if (realFiveOnFive[0][2 * counterArray[0][2 * j] + 1] - realFiveOnFive[0][2 * counterArray[0][2 * j]] === 300 ) {
                tempArray13.push(counterArray[0][2 * j]); tempArray10.push(counterArray[0][2 * j])}
                ...
                for (j = 0; j < counterArray[1].length / 2; j++) { 
                  if (counterArray[1][2 * j + 1] === tempArray9[i]) { // console.log(j, counterArray[1][2 * j], tempArray9[i]);
                    ...
                    tempArray1.push(counterArray[1][2 * j]); tempArray12.push(counterArray[1][2 * j]); // console.log('2 minutes', counterArray[1][2*j]);
                    } 
              ...
              for (i = tempArray9.length - 1; i > -1; i--) { if (counterArray[2][2*i].length >= counterArray[3][2*i].length) 
                { tempArray11 = counterArray[2][2*i].slice(0,counterArray[3][2*i].length);
                  tempArray12 = counterArray[3][2*i] }
                  else if (counterArray[2][2*i].length < counterArray[3][2*i].length)
                  {tempArray11 = counterArray[2][2*i];
                  tempArray12 = counterArray[3][2*i].slice(0,counterArray[2][2*i].length)}
                  if (counterArray[2][2*i + 1].length >= counterArray[3][2*i + 1].length) 
                { tempArray13 = counterArray[2][2*i + 1].slice(0,counterArray[3][2*i + 1].length);
                  ...
                  tempArray14 = counterArray[3][2*i + 1].slice(0,counterArray[2][2*i + 1].length)}
                  ...
                  tempArray8.sort(function(a, b){return a - b});
                    ...
                    realFiveOnFiveBefore = realFiveOnFive2[0].slice(0, 2 * tempArray7[j]);
                    realFiveOnFiveAfter = realFiveOnFive2[0].slice(2 * tempArray7[j] + 2);
                    realFiveOnFive2[0] = realFiveOnFiveBefore.concat(realFiveOnFiveAfter);
                    realFiveOnFiveBefore = realFiveOnFive2[1].slice(0, 2 * tempArray8[j]);
                    realFiveOnFiveAfter = realFiveOnFive2[1].slice(2 * tempArray8[j] + 2);
              ...
              function getDPairs() {shiftsArray = []; shiftsFArray = []; awayShiftsArray = []; awayShiftsFArray = [];
                ...
                {for (k = j + 1; k < shiftsArray2[i].length/2; k++) {tempTime = [];
                  for (l = 0; l < shiftsArray2[j].length/2; l++)
                  for (m = 0; m < shiftsArray2[k].length; m++)
                  {if ((shiftsArray2[i][k][2*m] >= shiftsArray2[i][j][2*l]) && (shiftsArray2[i][k] <= shiftsArray2[i][j])) {
                    if (shiftsArray2[i] >= shiftsArray2[j][2*l + 1]) {tempTime.push(shiftsArray2[i][j][2 * l + 1] )}
                    else {tempTime.push(shiftsArray2[i].push(shiftsArray2[i][k][2 * m + 1] - shiftsArray2[i][k][2 * m]))}                    
                  }
                  else if ((shiftsArray2[i][k][2 * m + 1] <= shiftsArray2[i][j][2 * l]) && (shiftsArray2[i]))
                  }
                }                  
                }
                }
